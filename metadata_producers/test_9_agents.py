...
def subdomains_agent(single_json: dict, 
                     language: str, 
                     gpt4o_encoder, 
                     gpt4o, 
                     prompt=subdomains_prompt,
                     reason=None,
                     count=0):
    text_message[0]["content"][0]["text"] = text_message[0]["content"][0]["text"].format(language)
    text_message[1]["content"][0]["text"] = prompt.format(language, reason, single_json["text"], single_json["sub_domains"])
    completion = gpt4o.chat.completions.create(
        messages=text_message, 
        model="gpt-4o-mini", 
        temperature=0.1
    ...
    if re.findall(r"<subdomains>(.*?)</subdomains>", llm_response, re.DOTALL): 
        ...
        if isinstance(single_json["sub_domains"], (list, dict)):
            print('i am here')
            isValid = json_validator(ans, "English", gpt4o_encoder, gpt4o)
            if(isValid["status"]==True):
                return ans
                ...
                if(count>=2):
                    ...
                    count+=1
                    ans = subdomains_agent(single_json, "English", gpt4o_encoder, gpt4o, reason=isValid["reason"], count=count+1)
